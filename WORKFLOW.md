...
treat it as documentation, not a source of truth (the code always wins if they
disagree).
...
| `grD`                     | Go to _declaration_ (not definition — e.g. C headers)                     |
...
filters, and a preview before applying. This is the go-to for anything beyond a
quick one-off substitution. `<C-S-f>` depends on your terminal/multiplexer
passing through the extended Ctrl+Shift keycode (same caveat as `<C-S-k>` above)
— `<leader>sR` always works regardless.
...
| `<leader>td` | Toggle diagnostics on/off (all buffers, all sources — LSP and nvim-lint) |
...
`~/.config/zellij/config.kdl` also unbinds the stock `Ctrl h` ("Move pane" mode)
and rebinds it to `Alt+m`, freeing `Ctrl+h` so it reaches Neovim as "previous
buffer" instead.
